...
def test_preprocessors_generic_session_mapping_filter_keys(generate_session_mapping_metrics: list[dict]):
    metrics = generate_session_mapping_metrics(10)
...
def test_preprocessors_generic_session_mapping_match_kv(generate_session_mapping_metrics: list[dict]):
    metrics = generate_session_mapping_metrics(10)
    ...
    kv_to_filter = secrets.choice(["src_ip", "dst_ip", "x_ip", "src_port", "dst_port", "x_port"])
...
def test_preprocessors_generic_session_mapping_key_exists_none(generate_session_mapping_metrics: list[dict]):
    metrics = generate_session_mapping_metrics(10)

    matched = key_exists(metrics, "fake_key")
    assert len(matched) == 0

def test_preprocessors_generic_key_exists_none(generate_session_mapping_metrics: list[dict]):
    metrics = generate_session_mapping_metrics(10)

    matched = key_exists(metrics, "x_port")
    assert len(matched) == 10

def test_preprocessors_generic_session_mapping_exclude_by_kv(generate_session_mapping_metrics: list[dict]):
    metrics = generate_session_mapping_metrics(10)
...
def test_preprocessors_generic_session_mapping_exclude_by_kvs(generate_session_mapping_metrics: list[dict]):
    metrics = generate_session_mapping_metrics(10)
    metric = secrets.choice(metrics)
...
def test_preprocessors_generic_session_mapping_exclude_by_kv_values(generate_session_mapping_metrics: list[dict]):
    metrics = generate_session_mapping_metrics(10)
    kv_to_filter = secrets.choice(["src_ip", "dst_ip", "x_ip", "src_port", "dst_port", "x_port"])
