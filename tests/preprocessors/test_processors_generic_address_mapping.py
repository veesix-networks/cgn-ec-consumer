...
def test_preprocessors_generic_address_mapping_filter_keys(generate_address_mapping_metrics: list[dict]):
    metrics = generate_address_mapping_metrics(10)
...
def test_preprocessors_generic_address_mapping_match_kv(generate_address_mapping_metrics: list[dict]):
    metrics = generate_address_mapping_metrics(10)
    ...
    kv_to_filter = secrets.choice(["src_ip", "x_ip"])
...
def test_preprocessors_generic_address_mapping_key_exists_none(generate_address_mapping_metrics: list[dict]):
    metrics = generate_address_mapping_metrics(10)

    matched = key_exists(metrics, "fake_key")
    assert len(matched) == 0

def test_preprocessors_generic_key_exists_none(generate_address_mapping_metrics: list[dict]):
    metrics = generate_address_mapping_metrics(10)

    matched = key_exists(metrics, "x_ip")
    assert len(matched) == 10

def test_preprocessors_generic_address_mapping_exclude_by_kv(generate_address_mapping_metrics: list[dict]):
    metrics = generate_address_mapping_metrics(10)
...
def test_preprocessors_generic_address_mapping_exclude_by_kvs(generate_address_mapping_metrics: list[dict]):
    metrics = generate_address_mapping_metrics(10)
    metric = secrets.choice(metrics)
...
def test_preprocessors_generic_address_mapping_exclude_by_kv_values(generate_address_mapping_metrics: list[dict]):
    metrics = generate_address_mapping_metrics(10)
    kv_to_filter = secrets.choice(["src_ip", "x_ip"])
